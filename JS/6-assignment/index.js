const analyzeUsers = (users) => {
    ...
    const allPopularPosts = users.flatMap(user => 
    ...
    const totalLikes = users.reduce(
        (sum, user) => sum + user.posts.reduce((postSum, post) => postSum + post.likes, 0),
        0
    );

    const avgLikesPerUser = users.length > 0 ? totalLikes / users.length : 0;

    return {
        activeUsersCount: activeUsers.length,
        totalPopularPosts: allPopularPosts.length, 
